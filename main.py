...
class MovieRecommendation:
    ...
    def create_supersets(self, superset_max_size=15):
        ...
        self.create_initial_freq_itemsets()
            ...
            for user_id, user_movies in self.users_movies.items():
                ...
                for itemset in self.frequent_itemsets[superset_length - 1]:
                    if itemset.issubset(user_movies):
                        # Construct next level superset based other favoriate movies of users exclude the current
                        # itemset itself
                        for other_reviewed_movie in user_movies - itemset:
                            current_superset = itemset | frozenset((other_reviewed_movie,))
                            # increase the frequency of recent superset which just occured
                            candidate_superset_freq[current_superset] += 1

            new_frequent_itemsets = dict([(itemset, frequency)
                                          for itemset, frequency in candidate_superset_freq.items()
        ...
        print("Total numer of itemsets in any length: {0}".format(
            sum(len(itemsets) for itemsets in self.frequent_itemsets.values())))
    ...
    def extract_association_rules(self):
        ...
        rule_confidence = {
            candidate_rule: valid_rule[candidate_rule] / float(
                valid_rule[candidate_rule] + invalid_rule[candidate_rule])
            for candidate_rule in candidate_rules}

        # Filter out the rules with poor confidence
        self.significant_rules = {rule: confidence for rule, confidence in rule_confidence.items() if
                                  confidence > self.min_confidence}

        print("In total {} rules found which {} of them are significant.".format(len(candidate_rules),
    ...
    def report_associations(self):
        ...
        for index in range(5):
            ...
            premise_names = ", ".join(self.get_movie_name(movie_id=mov_id) for mov_id in premise)
            conclusion_name = self.get_movie_name(movie_id=conclusion)

            print(
                "Rule: If a person recommends {0} they will also recommend {1}".format(premise_names, conclusion_name))
...
if __name__ == '__main__':
    ...
    engine.create_supersets(superset_max_size=5)
